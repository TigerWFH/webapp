...
[参考资料](https://github.com/yaofly2012/note/issues/112)
[原始参考资料](https://juejin.cn/post/6844903762478235656)
...
/*
        ...
        -----------------------------------------------------------------------------------------------
            ...
            ES2015 Promise：
                ES2015 Promise是对Promise A+规范的实现，且严格遵守该规范。Promise代表异步操作的结果
                Promise.resolve(value)：返回一个fulfilled的Promise
                    如果value是个thenable对象非Promise对象，则会进行promise resolution procedure，此时Promise.resolve(value) 等价 new Promise((resolve) => resolve(value))

                    如果value是Promise对象，非thenable对象，则直接返回实参，此时Promise.resolve 不等价 new Promise(resolve => resolve(value))
                Promise.reject(value)完全和new Promise((undefined, reject) => reject(value))等价
        ...
        ---------------------------------------------------------------------------------------------
            ...
            实际问题：见JS代码
                Promise.resolve(v)和new Promise((resolve) => resolve(v))不完全一样，
                如果v是一个promise对象，Promise.resolve直接返回；但是new Promise((resolve) => resolve())需要做PromiseResolutionProcedure，该步骤是一个异步过程。
...
var p1 = Promise.resolve(p); // 会进行PromiseResolveThenableJob
p1.then(() => console.log("p1.1"));
p.then(() => console.log("p.1"))
  .then(() => console.log("p.2"))
  .then(() => console.log("p.3"));
// 输出结果：p.1,p.2,p1.1,p.3
// 现象：p1状态被延迟了两个时序
// 原因：p是一个对象，需要异步创建一个新Promise对象，延迟一个时序；通过p.then获取p的终态，又延迟一个时序；然后获取p1的终态
// 实际问题2，x是thenable，非promise对象
var thenable = {
  then: (resolve) => {
    console.log("thanable.then"), resolve();
  },
};
var p = Promise.resolve("p");
...
p.then(() => console.log("p.1"))
  .then(() => console.log("p.2"))
  .then(() => console.log("p.3"));
// 输出结果：p.1,p1.1,p.2,p.3
// 现象：p1状态延迟了一个时序
// 原因：p是一个thenable对象，需要异步创建以恶新Promise对象，延迟一个时序；thenable状态能够直接获取，不需要延迟
```
